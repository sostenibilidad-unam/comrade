...
public class UrbanizandoAntiguo extends Automata{
...
//	public int numeroDeColumnas;
	...
	public UrbanizandoAntiguo(int numeroDeCeldasX, int numeroDeCeldasY, int numeroMaximoDeIteraciones, int estadosPosibles) {
		...
		infoADN = new CodigoGenetico(10);
		infoADN.setRango(0, 3, 17);
		infoADN.setRango(1, 3, 17);
		infoADN.setRango(2, 3, 17);
		infoADN.setRango(3, 100, 333);
		infoADN.setRango(4, 100, 333);
		infoADN.setRango(5, 100, 333);
		infoADN.setRango(6, 40, 160);
		infoADN.setRango(7, 40, 160);
		infoADN.setRango(8, 100, 333);
		infoADN.setRango(9, 700, 1000);
//		infoADN.setRango(10, 800, 1000);
//		infoADN.setRango(11, 1, 555);
		
	
		
		nombresDeLosParametros = new String[10];
		nombresDeLosParametros[0]="vecino_0";
		nombresDeLosParametros[1]="vecino_carretera_0";
		nombresDeLosParametros[2]="vecino_pueblo";
		nombresDeLosParametros[3]="buffer_carretera_0";
		nombresDeLosParametros[4]="buffer_pueblo";
		nombresDeLosParametros[5]="buffer_carr_pueb";
		nombresDeLosParametros[6]="vecino_1";
		nombresDeLosParametros[7]="vecino_carretera_1";
		nombresDeLosParametros[8]="buffer_carretera_1";
		nombresDeLosParametros[9]="vecino_2";
    ...
    public void reglasDeTranciscion(int i,int j){
    	...
    	if (elMisterioHumano.nextInt(100)>80){
    		eras=valor[i][j][iteracion];
    		
    		//*********************************aqui se puede modificar la forma de tomar en cuenta 
    		//*********************************los vecinos
    		//losVecinos=laSumaPesada(i, j);
    		losVecinos=laSumaCiv(i, j);
    		//reglas de transicion para cambiar de 0 a 1  
    		
    		if (eras == 0) {
    			if ( dificultad[i][j]>0.4 ){
    				valor[i][j][iteracion+1] = 0;
    			}else if (losVecinos >= vecino_0){
	    			valor[i][j][iteracion+1] = 1; // difusion por vecino
	    		}else if( (losVecinos >= vecino_carretera_0) && (dist_vias[i][j] <= buffer_carretera_0)){
	    					valor[i][j][iteracion+1] = 1; // dispersion cercana a la carretera
	     		}else if( (losVecinos >= vecino_pueblo) && ( distanciaAlaMancha(i,j,iteracion) <= buffer_pueblo) && ( dist_vias[i][j] <= buffer_carr_pueb)){
	    					valor[i][j][iteracion+1] = 1; // dispersion cercana al pueblo
	    		}else valor[i][j][iteracion+1] = 0;
//    			reglas de transicion para cambiar de 1 a 2  
    		}
    		if (eras == 1){
        		if (losVecinos >= vecino_1){
    				valor[i][j][iteracion+1] = 10;// difusion (engorda los 2’s)
    			}else if( (losVecinos >= vecino_carretera_1) && (dist_vias[i][j] <= buffer_carretera_1)){
        	    		valor[i][j][iteracion+1] = 10; // dispersion cercana a la carretera
        	    }else valor[i][j][iteracion+1] = 1;
        	}
    		//reglas de transicion para cambiar de 10 a 100
    		if (eras == 10){
    			if (losVecinos >= vecino_2){
...
//    	} 
	...
	public double mide(int[] esteADN){
    	return mide(esteADN[0], esteADN[1], esteADN[2], esteADN[3], 
    			esteADN[4], esteADN[5], esteADN[6], esteADN[7], esteADN[8], esteADN[9]);
    ...
    public double mide(int vecino_0, int vecino_carretera_0, int vecino_pueblo,
    		...
    		int vecino_1, int vecino_carretera_1, int buffer_carretera_1,
    		int vecino_2){
    	//este metodo usa 12 parametros pero no debe ser dificil programarlo para que 
    	//el numero de parametros lo obtenga del codigoGenetico que aqui se llama infoADN
    	//por otro lado CodigoGenetico debe llamarse de otro modo para que al usar otros 
	...
	public int[][] unOutputPara(int[] parametros) {
    	...
    	this.vecino_carretera_1=parametros[7];
    	this.buffer_carretera_1=parametros[8];
    	this.vecino_2=parametros[9];
